...
ADD __init__.py /CNN_SM
RUN mkdir libs
ADD libs /CNN_SM/libs
RUN mkdir ssh
ADD ssh /CNN_SM/ssh
RUN mkdir sources
ADD sources /CNN_SM/sources
RUN mkdir nets
ADD nets /CNN_SM/nets

RUN mkdir config
ADD config /CNN_SM/config

ADD CNN.jpeg /CNN_SM

RUN mkdir Singularity_le
ADD Singularity_le /CNN_SM/Singularity_le


ADD logonic.png /CNN_SM
ADD training_script.py /CNN_SM
ADD CNN_GUI.py /CNN_SM
ADD inference_script.py /CNN_SM
ADD preprocess_inference_script.py /CNN_SM
ADD cnn_main.py /CNN_SM
RUN mkdir tensorboardlogs
ADD tensorboardlogs /CNN_SM/tensorboardlogs
ADD requirements.txt /CNN_SM
# add permissions (odd)
