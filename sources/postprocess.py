...
def invert_registration(current_folder, options):
    ...
    try:
        ...
        print("Computing the inverse transformation using:", CYELLOW + "reg_transform " + CEND, CRED + "[Step 1]" + CEND)
    ...
    try:
            # reg_resample - ref
            # Flair_reg_brain_bias.nii.gz - flo. / PITMS - seg - v3 / PITMS - seg - v3_CNN_final_segmentation.nii.gz - trans
            # inv_transf.txt - res
            # mask_BacktoOriginalSpace.nii.gz - inter
            # 0

            # subprocess.check_output([reg_resample_path,
            #                          '-ref', os.path.join(options['tmp_folder'],
            #                                               'FLAIR.nii.gz'),
            #                          '-flo', 'CNN_final_segmentation.nii.gz',
            ...
            print('\x1b[6;30;42m' + '..................After applying final transformations!...................' + '\x1b[0m')
            print('\x1b[6;30;42m' + 'Predicted segmentation output can be found in Model* folder under the name' + '\x1b[0m')
