...
def parse_input_masks(current_folder, options):
    ...
    print('masks:', masks)
    input_path = os.path.join(current_folder, masks[0])
    print('copying', str(masks[0]), 'into folder --->', options['tmp_folder'])
    ...
    #         print('copying', str(m), 'into folder --->', options['tmp_folder'])
    #         if m == 'FLAIR':
    #
    #             shutil.copy(input_path,
    #                     os.path.join(options['tmp_folder'],
...
def preprocess_scan(current_folder, options):
    ...
    if options['use_of_fsl'] is True:
            ...
            for mod in options['modalities']:
                ...
                # else:
                    if which('reg_aladin') is not None:
                        print(CRED2 + "Using installed reg_aladin!" + CEND)
                        print("registering using ", CYELLOW + "reg_aladin" + CEND,  mod, '---> MNI152_T1_1mm.nii.gz')
                        print('Current version of niftyreg:')
                        os.system('reg_f3d --version')
                        # subprocess.check_output(['reg_aladin', '-ref',
                        ...
                        process = subprocess.Popen(['reg_aladin', '-ref',
                                             os.path.join(options['le_path']),
                                             '-flo', os.path.join(options['tmp_folder'], 'flair_copy.nii'),
                                             '-aff', os.path.join(options['tmp_folder'], mod + '_transf.txt'),
                                             '-res', os.path.join(options['tmp_folder'], 'r' + mod + '.nii.gz')],
                                                   stdout=subprocess.PIPE,
                                                   universal_newlines=True)
                        ...
                        print(CRED + "Using CNN reg_aladin!" + CEND)
                        # print('Current version of niftyreg:')
                        # os.system('reg_f3d --version')
                        os_host = platform.system()
                        reg_exe = ''
                        ...
                        #                      '-aff', os.path.join(options['tmp_folder'], mod + '_transf.txt'),
                        #                      '-res', os.path.join(options['tmp_folder'], 'r' + mod + '.nii.gz')])
                        process = subprocess.Popen([reg_aladin_path, '-ref',
                                             os.path.join(options['le_path']),
                                             '-flo', os.path.join(options['tmp_folder'], 'flair_copy.nii'),
                                             '-aff', os.path.join(options['tmp_folder'], mod + '_transf.txt'),
                                             '-res', os.path.join(options['tmp_folder'], 'r' + mod + '.nii.gz')],
            ...
            if options['rf'] is True:
                print("skull stripping using:", CYELLOW + "fsl_bet" + CEND)
            ...
            #     os.chdir(folder_path)
            #     os.system('bet  rFLAIR.nii.gz  FLAIR_t_x.nii.gz  -f 0.5 -g 0')

                # folder_pathy = os.path.join(os.path.join(options['fsl__bet_sh']))
                # os.chdir(folder_pathy)
                # os.system('bash  fsl.sh')
                # folder_pathx = os.path.join(os.path.join(options['fsl__bet_path']))
                # os.chdir(folder_pathx)
                # print('bet', bet)
                print('Bet path:', CYELLOW + which('bet') + CEND)
                print('Bet2 path:', CYELLOW + which('bet2') + CEND)
                # print('fsl-bet path:', CYELLOW + which('fsl-bet') + CEND)
                # print('fsl5.0-bet2 path:', CYELLOW + which('fsl5.0-bet2') + CEND)
                bet = 'bet2'
                ...
                #                            t1_im,
                #                            t1_st_im])
                process = subprocess.Popen(["bash", options['robex_path'],
                                           t1_im,
                                           t1_st_im],
                                           stdout=subprocess.PIPE,
                                           universal_newlines=True)
